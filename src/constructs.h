...
struct LAInput: public LANeuron
{
	...
	int SetSpikes(vector<int>&  spiketimings)
	{
		Reset();
		if (spiketimings.size() >0)
			spikeTimes = (spiketimings);

		this->curSpike =0;
		if (this->spikeTimes.size() > 0)
			this->nextSpikeT = this->spikeTimes.at(this->curSpike);

		this->totalSpikes = spikeTimes.size();
	...
	int Program(int tstart, int duration, float freq, float randomness)
	{
		...
		{
			spikeTimes.push_back (  (int) ( tstart + period*i + rgen()*randomness*period ) ) ;
		}
...
class LANetwork
{
	...
	vector<LANeuron*> pattern_inputs;

	vector< vector<LANeuron*> > inputs_cs;
